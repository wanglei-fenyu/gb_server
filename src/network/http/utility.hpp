...
namespace gb::http {
  ...
  // 基于 https://stackoverflow.com/questions/2590677/how-do-i-combine-hash-values-in-c0x/2595226#2595226
  ...
  public:
    ...
    static std::string encode(const std::string &value) noexcept {
      ...
      std::string result;
      result.reserve(value.size()); // 结果的最小大小
    ...
    static std::string decode(const std::string &value) noexcept {
      std::string result;
      result.reserve(value.size() / 3 + (value.size() % 3)); // 结果的最小大小
  ...
  public:
    ...
    public:
      ...
      public:
        /// 解析 Set-Cookie 或 Content-Disposition 头字段值。属性值会被百分号解码。
...
namespace gb::http {
  /// 使得无需停止 asio::io_context 即可取消 Asio 处理器等
  class ScopeRunner {
    /// 作用域计数，若作用域被取消则置为 -1
