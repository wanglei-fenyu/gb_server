...
namespace gb::http {
  ...
  protected:
    ...
    std::shared_ptr<Connection> create_connection() noexcept override {
      return std::make_shared<Connection>(handler_runner, config.timeout, *io_context, context);
    }
      ...
      if(!session->connection->socket->lowest_layer().is_open()) {
        ...
        resolver->async_resolve(query_host, query_service, [this, session, resolver](const error_code &ec, asio::ip::tcp::resolver::results_type results) {
          ...
          if(!ec) {
            ...
            asio::async_connect(session->connection->socket->lowest_layer(), results, [this, session, resolver](const error_code &ec, asio::ip::tcp::endpoint /*endpoint*/) {
